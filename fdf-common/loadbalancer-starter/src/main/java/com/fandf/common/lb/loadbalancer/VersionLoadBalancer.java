package com.fandf.common.lb.loadbalancer;

import com.fandf.common.constant.CommonConstant;
import com.fandf.common.lb.chooser.IRuleChooser;
import com.fandf.common.lb.utils.QueryUtils;
...
public class VersionLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    private String getVersionFromRequestData(RequestData requestData){
        ...
        if(MapUtils.isNotEmpty(queryMap)&& queryMap.containsKey(CommonConstant.L_T_VERSION)&& StringUtils.isNotBlank(queryMap.get(CommonConstant.L_T_VERSION))){
            return queryMap.get(CommonConstant.L_T_VERSION);
        }else if(requestData.getHeaders().containsKey(CommonConstant.L_T_VERSION)){
            return requestData.getHeaders().get(CommonConstant.L_T_VERSION).get(0);
        }
        return null;
    }
